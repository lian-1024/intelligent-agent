...
class ZhipuAIStrategy(ModelProviderStrategy):
    ...
    @override
    def load_model(self, model: str):
        return ChatZhipuAI(model=model, api_key=settings.ZHIPUAI_API_KEY)
...
def load_chat_model(
    model_and_provider: str, separator: Optional[str] = ":"
) -> BaseChatModel:
    """
    模型加载
    """
    ...
    strategies: dict[str, ModelProviderStrategy] = {
    ...
    if provider not in strategies:
        raise ValueError(
            f"不支持的提供商: {provider}，当前支持的提供商有: {', '.join(strategies.keys())}"
        )
